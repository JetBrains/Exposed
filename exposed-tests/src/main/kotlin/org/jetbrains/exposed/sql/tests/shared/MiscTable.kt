...
fun MiscTable.checkRow(
    ...
    assertEquals(by, row[this.by])
    assertEquals(byn, row[this.byn])
    assertEquals(sm, row[this.sm])
    assertEquals(smn, row[this.smn])
    assertEquals(n, row[this.n])
    assertEquals(nn, row[this.nn])
    assertEquals(e, row[this.e])
    assertEquals(en, row[this.en])
    assertEquals(es, row[this.es])
    assertEquals(esn, row[this.esn])
    assertEquals(c, row[this.c])
    assertEquals(cn, row[this.cn])
    assertEquals(s, row[this.s])
    assertEquals(sn, row[this.sn])
    assertEquals(dc, row[this.dc])
    assertEquals(dcn, row[this.dcn])
    assertEquals(fcn, row[this.fcn])
    assertEquals(dblcn, row[this.dblcn])
}
