 ...
 * ```
 *
 * Using the default argument for [hasher] requires calling `hash()` directly on the column, as that is the only
 * way to reach the underlying default [Hasher] implementation:
 *
 * ```kotlin
 * object Users : IntIdTable() {
 ...
 *
 * val user = Users.selectAll().where { Users.id eq id }.single()
 * val granted = user[Users.recoveryCode]?.matches(submittedCode) == true
 * ```
 *
 * Using the default argument for [hasher] requires calling `hash()` directly on the column, as that is the only
 * way to reach the underlying default [Hasher] implementation:
