...
class DefaultsTest : DatabaseTestsBase() {
    ...
    fun testDefaults01() {
        ...
        val TestTable = object : IntIdTable("t") {
            ...
            val t1 = datetime("t1", UTC_ZONE_ID).defaultExpression(currentDT)
            val t2 = datetime("t2", UTC_ZONE_ID).defaultExpression(nowExpression)
            val t3 = datetime("t3", UTC_ZONE_ID).defaultExpression(dtLiteral)
            val t4 = date("t4", UTC_ZONE_ID).default(dtConstValue)
            val t5 = timestamp("t5", UTC_ZONE_ID).default(tsConstValue)
            val t6 = timestamp("t6", UTC_ZONE_ID).defaultExpression(tsLiteral)
            val t7 = duration("t7").default(durConstValue)
            val t8 = duration("t8").defaultExpression(durLiteral)
            val t9 = time("t9", UTC_ZONE_ID).default(tmConstValue)
            val t10 = time("t10", UTC_ZONE_ID).defaultExpression(tLiteral)
        }
