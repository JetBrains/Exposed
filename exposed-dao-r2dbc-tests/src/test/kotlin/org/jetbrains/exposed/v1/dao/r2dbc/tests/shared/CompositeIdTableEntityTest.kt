...
class CompositeIdTableEntityTest : R2dbcDatabaseTestsBase() {
    ...
    class Author(id: EntityID<Int>) : IntEntity(id) {
        ...
        val publisher by Publisher referencedOn Authors
    ...
    class Book(id: EntityID<CompositeID>) : CompositeEntity(id) {
        ...
        val author by Author optionalReferencedOn Books.author
    ...
    class Review(id: EntityID<CompositeID>) : CompositeEntity(id) {
        ...
        // R2DBC: relationship is mutated via `book.set(bookValue)` on the accessor.
        val book by Book referencedOn Reviews
    }
    ...
    class Office(id: EntityID<CompositeID>) : CompositeEntity(id) {
        ...
        val publisher by Publisher optionalReferencedOn Offices
    }
    ...
    fun testInsertAndSelectUsingDAO() {
        withTables(Publishers) {
            ...
            Publisher.newSuspend { name = "Publisher B" }
            Publisher.newSuspend { name = "Publisher C" }
    ...
    fun testInsertWithCompositeIdAutoGeneratedPartsUsingDAO() {
        ...
        withTables(excludeSettings = listOf(TestDB.SQLSERVER), Publishers) {
            ...
            val isbn = Uuid.random()
            val p2 = Publisher.newSuspend(
                CompositeID {
    ...
    fun testInsertWithCompositeIdAutoGeneratedPartsAndMissingNotGeneratedPartUsingDAO() {
        withTables(tables = allTables) {
            val publisherA = Publisher.newSuspend {
                name = "Publisher A"
            }
            val authorA = Author.newSuspend {
                publisher.set(publisherA)
                penName = "Author A"
            }
            val bookA = Book.newSuspend {
                title = "Book A"
                author.set(authorA)
            }
    ...
    fun testInsertAndGetCompositeIds() {
        withTables(excludeSettings = listOf(TestDB.SQLSERVER), Publishers) {
            ...
            val id5: EntityID<CompositeID> = Publishers.insertAndGetId {
                it[id] = CompositeID { id ->
                    id[pubId] = 1001
                }
                it[name] = "Publisher C"
            }
            assertEquals(1001, id5.value[Publishers.pubId].value)

            // insert as partially filled composite ID with autoincrement part
            val id6: EntityID<CompositeID> = Publishers.insertAndGetId {
                it[id] = CompositeID { id ->
                    id[isbn] = Uuid.random()
                }
                it[name] = "Publisher C"
    ...
    fun testInsertUsingManualCompositeIds() {
        withTables(excludeSettings = listOf(TestDB.SQLSERVER), Publishers) {
            ...
            val fullId = CompositeID {
                ...
                it[Publishers.isbn] = Uuid.random()
            }
            val p2Id = Publisher.newSuspend(fullId) {
                name = "Publisher B"
            }.id
    ...
    fun testFindByCompositeId() {
        withTables(excludeSettings = listOf(TestDB.SQLSERVER), Publishers) {
            ...
            val id2: EntityID<CompositeID> = Publisher.newSuspend {
                name = "Publisher B"
            }.id
    ...
    fun testInsertAndSelectReferencedEntities() {
        withTables(tables = allTables) {
            ...
            val reviewA: Review = Review.newSuspend(reviewIdValue) {
                book.set(bookA)
            }
    ...
    fun testPreloadBackReferencedOn() {
        withTables(tables = allTables) {
            ...
            Office.newSuspend(officeAIdValue) {}
            val officeBIdValue = CompositeID {
                it[Offices.zipCode] = "5C6 7D8"
                it[Offices.name] = "Office B"
                it[Offices.areaCode] = 456
