...
class EntityCacheRefreshTests : R2dbcDatabaseTestsBase() {
    // SQL Server has no `SELECT ... FOR UPDATE`; it uses locking hints instead, so the
    // row-locking tests below cannot express the same contention there.
