...
class UuidTableEntityTest : R2dbcDatabaseTestsBase() {
    ...
    fun `create records`() {
        withTables(UuidTables.Cities, UuidTables.People) {
            val mumbai = UuidTables.City.newSuspend { name = "Mumbai" }
            val pune = UuidTables.City.newSuspend { name = "Pune" }
            UuidTables.Person.newSuspend(Uuid.random()) {
                name = "David D'souza"
                city.set(mumbai)
            }
            UuidTables.Person.newSuspend(Uuid.random()) {
    ...
    fun `update and delete records`() {
        withTables(UuidTables.Cities, UuidTables.People) {
            val mumbai = UuidTables.City.newSuspend(Uuid.random()) { name = "Mumbai" }
            val pune = UuidTables.City.newSuspend(Uuid.random()) { name = "Pune" }
            UuidTables.Person.newSuspend(Uuid.random()) {
                name = "David D'souza"
                city.set(mumbai)
            }
            ...
            val tanu = UuidTables.Person.newSuspend(Uuid.random()) {
                name = "Tanu Arora"
                city.set(pune)
            }
    ...
    fun `insert with inner table`() {
        withTables(UuidTables.Addresses, UuidTables.Cities, UuidTables.People) {
            val city1 = UuidTables.City.newSuspend {
                name = "city1"
            }
            val person1 = UuidTables.Person.newSuspend {
                name = "person1"
                city.set(city1)
            }

            val address1 = UuidTables.Address.newSuspend {
                person.set(person1)
                city.set(city1)
                address = "address1"
            }

            val address2 = UuidTables.Address.newSuspend {
                person.set(person1)
                city.set(city1)
                address = "address2"
            }
    ...
    fun testUuidVersionAutoGenerated() {
        withTables(UuidTables.Cities, UuidTables.Books) {
            ...
            val book2 = UuidTables.Book.newSuspend {
