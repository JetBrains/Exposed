...
class R2dbcSequencesTests : R2dbcDatabaseTestsBase() {
    object TesterTable : UuidTable("Tester") {
        val index = integer("index").autoIncrement()
        val name = text("name")
    }

    class TesterEntity(id: EntityID<Uuid>) : UuidR2dbcEntity(id) {
        companion object : UuidR2dbcEntityClass<TesterEntity>(TesterTable)
