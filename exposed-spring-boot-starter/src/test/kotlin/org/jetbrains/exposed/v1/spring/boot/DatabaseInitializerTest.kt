...
@SpringBootTest(
    ...
    properties = [
        "spring.autoconfigure.exclude=org.jetbrains.exposed.v1.spring.boot.autoconfigure.ExposedAutoConfiguration",
        "spring.exposed.excluded-packages=org.jetbrains.exposed.v1.spring.boot.tables.ignore"
    ]
)
open class DatabaseInitializerTest {
    ...
    fun `ignore non object Table`() {
        Database.connect("jdbc:h2:mem:test-spring", user = "sa", driver = "org.h2.Driver")
        val tables = ExposedAutoConfiguration.discoverExposedTables(applicationContext, listOf())
