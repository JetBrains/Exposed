...
fun Application.seedRoutes() {
    routing {
        post("/seed") {
            suspendTransaction {
                // `newSuspend { }` flushes immediately, so it costs one INSERT per entity — that is
                // the right default, and every other creation below uses it.
                //
                ...
                // then go out as a single batched INSERT. The next `newSuspend { }` flushes them, which
                // is also where the tags get their ids — nothing reads those before that point.
