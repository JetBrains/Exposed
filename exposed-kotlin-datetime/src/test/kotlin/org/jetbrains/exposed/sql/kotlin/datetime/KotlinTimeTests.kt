...
private fun assertEqualFractionalPart(nano1: Int, nano2: Int) {
    ...
    when (dialect) {
        ...
        is SQLServerDialect, is SQLiteDialect, is H2Dialect, is OracleDialect ->
            assertEquals(roundTo100Nanos(nano1), roundTo100Nanos(nano2), "Failed on 1/10th microseconds $db")
        // microseconds
        is MariaDBDialect -> when ((dialect as? MariaDBDialect)?.isFractionDateTimeSupported()) {
            true -> assertEquals(floorToMicro(nano1), floorToMicro(nano2), "Failed on microseconds $db")
            else -> {} // don't compare fractional part
        }
        is PostgreSQLDialect, is MysqlDialect -> {
