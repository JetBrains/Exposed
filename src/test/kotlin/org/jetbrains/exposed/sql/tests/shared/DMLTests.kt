...
class DMLTests : DatabaseTestsBase() {
  ...
  @Test fun testSelect() {
    withCitiesAndUsers { cities, users, userData ->
      users.
          select{users.id.eq("andrey")}.forEach {
  ...
  @Test fun testSelectAnd() {
    withCitiesAndUsers { cities, users, userData ->
      users.
          select{users.id.eq("andrey") and users.name.eq("Andrey")}.forEach {
        val userId = it[users.id]
        val userName = it[users.name]
        when (userId) {
          "andrey" -> assertEquals("Andrey", userName)
  ...
  @Test fun testSelectOr() {
    withCitiesAndUsers { cities, users, userData ->
      users.
          select{users.id.eq("andrey") or users.name.eq("Andrey")}.forEach {
        val userId = it[users.id]
        val userName = it[users.name]
        when (userId) {
          "andrey" -> assertEquals("Andrey", userName)
  ...
  @Test fun testSelectNot() {
    withCitiesAndUsers { cities, users, userData ->
      users.
          select{org.jetbrains.exposed.sql.not(users.id.eq("andrey"))}.forEach {
        val userId = it[users.id]
        val userName = it[users.name]
        if (userId == "andrey") {
  ...
  @Test fun testInsert03() {
    ...
    withTables(tbl) {
      ...
      val row = tbl.selectAll().single()
      tbl.checkRow(row, 42, 42, date, date, time, time, DMLTestsData.E.ONE, DMLTestsData.E.ONE, DMLTestsData.E.ONE, DMLTestsData.E.ONE, "test", "test", BigDecimal("239.42"), BigDecimal("239.42"))
    }
  }
  ...
  @Test fun testInsert04() {
    ...
    withTables(tbl) {
      ...
      val row = tbl.selectAll().single()
      tbl.checkRow(row, 42, null, date, null, time, null, DMLTestsData.E.ONE, null, DMLTestsData.E.ONE, null, stringThatNeedsEscaping, null, BigDecimal("239.42"), null)
    }
  }
...
*/
  ...
  @Test fun testUpdate02() {
    ...
    withTables(tbl) {
      ...
      val row = tbl.selectAll().single()
      tbl.checkRow(row, 42, null, date, null, time, null, eOne, null, eOne, null, sTest, null, dec, null)
    }
  }
  ...
  @Test fun testUpdate03() {
    ...
    withTables(excludeSettings = listOf(TestDB.MYSQL), tables = tbl) {
      ...
      tbl.checkRow(row, 101, null, date, null, time, null, eOne, null, eOne, null, "23456789", "3456789", dec, null)
    }
  }
