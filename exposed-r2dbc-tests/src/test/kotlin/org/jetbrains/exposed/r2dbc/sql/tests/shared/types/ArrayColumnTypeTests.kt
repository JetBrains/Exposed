...
class ArrayColumnTypeTests : R2dbcDatabaseTestsBase() {
    ...
    fun testSelectUsingArraySlice() {
        withTestTableAndExcludeSettings {
            ...
            if (org.jetbrains.exposed.sql.vendors.currentDialect is H2Dialect) { // H2 returns SQL NULL if any parameter in ARRAY_SLICE is null
                assertNull(result2)
            } else {
            ...
            if (org.jetbrains.exposed.sql.vendors.currentDialect is H2Dialect) {
                assertNull(result3)
            } else {
