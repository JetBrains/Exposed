        ...
        withTables(excludeSettings = TestDB.ALL_MYSQL + TestDB.ALL_MARIADB + TestDB.ALL_ORACLE_LIKE, tables = arrayOf(testTable)) {
            ...
            assertEquals(
                "CREATE TABLE " +
                    ...
                    "\"${"name".inProperCase()}\" $varCharType PRIMARY KEY, " + "${"age".inProperCase()} ${currentDialectTest.dataTypeProvider.integerType()} NULL)",
                testTable.ddl
            )
        }
    }
...
//        }
    ...
    fun testIndexWithFunctions() {
        ...
        withDb { testDb ->
            ...
            val uniqueIndexName = "tester_price_coalesce${"_unique"}".inProperCase()
            ...
            val functionStrings = when (testDb) {
                TestDB.ORACLE -> listOf("(amount + price)", "LOWER(item)", "COALESCE(item, '*')").map(String::inProperCase)
                    ...
                    tester.amount.plus(tester.price).toString(), "$p1${tester.item.lowerCase()}$p2", "$p1${Coalesce(tester.item, stringLiteral("*"))}$p2"
                )
            }
    ...
    fun testBinaryWithoutLength() {
        ...
        fun Query.readAsString() = map { String(it[tableWithBinary.binaryColumn]) }

        val exposedBytes = "Exposed".toByteArray()
        val kotlinBytes = "Kotlin".toByteArray()

        withTables(excludeSettings = TestDB.ALL - listOf(TestDB.POSTGRESQL, TestDB.H2_V2_PSQL, TestDB.H2_V2), tableWithBinary) {
    ...
    fun testBinary() {
        ...
        withTables(t) {
            ...
            assertEqualCollections(t.selectAll().readAsString(), listOf("Hello!", "World!"))
            ...
            assertEqualCollections(t.selectAll().readAsString(), listOf("Hello!", "World!", null))

            val world = t.selectAll().where { t.binary eq worldBytes }.readAsString()
            assertEqualCollections(world, listOf("World!"))

            val worldByBitCol = t.selectAll().where { t.byteCol eq byteArrayOf(1) }.readAsString()
            assertEqualCollections(worldByBitCol, listOf("World!"))
        }
    }
    ...
    fun tableWithDifferentTextTypes() {
        ...
        withTables(excludeSettings = TestDB.ALL - listOf(TestDB.POSTGRESQL, TestDB.MYSQL_V5, TestDB.H2_V2_PSQL), testTable) { testDb ->
            assertEquals(
                "CREATE TABLE " +
                    addIfNotExistsIfSupported() +
                    "${"different_text_column_types".inProperCase()} " +
    ...
    fun testCreateAndDropCheckConstraint() {
        ...
        withTables(tester) { testDb ->
            ...
            if (testDb in listOf(TestDB.MYSQL_V5)) { // cannot alter existing check constraint
    ...
    fun testMultipleFK() {
        ...
        withTables(tableA, tableB, tableC) {
            tableA.insert {
                it[idA] = 1
                it[idB] = 2
            }
