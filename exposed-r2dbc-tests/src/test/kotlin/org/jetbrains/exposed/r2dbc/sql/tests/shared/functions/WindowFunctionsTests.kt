...
class WindowFunctionsTests : R2dbcDatabaseTestsBase() {

    private val supportsCountDistinctAsWindowFunction = TestDB.ALL_H2 + TestDB.ORACLE
    private val supportsStatisticsAggregateFunctions = TestDB.ALL - listOf(TestDB.SQLSERVER)
    ...
    private suspend fun <T> DMLTestsData.Sales.assertWindowFunctionDefinition(
        definition: WindowFunctionDefinition<T>,
        expectedResult: List<T>
    ) {
        val result = select(definition)
            .orderBy(
                year to org.jetbrains.exposed.sql.SortOrder.ASC,
                month to org.jetbrains.exposed.sql.SortOrder.ASC,
                product to org.jetbrains.exposed.sql.SortOrder.ASC_NULLS_FIRST
            )
            .map { it[definition] }
