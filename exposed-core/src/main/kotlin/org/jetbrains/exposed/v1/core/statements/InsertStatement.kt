...
open class InsertStatement<Key : Any>(
     ...
     *
     * `null` indicates that this statement has not been executed yet. The rows are built from the submitted
     * insert values together with generated, returned, or default column values when available.
     *
    ...
    infix operator fun <T> get(column: Column<T>): T {
        ...
        if (!row.hasValue(column) && column.columnType.isAutoInc && dialect?.supportsGeneratedKeysRetrieval == false) {
            throw UnsupportedByDialectException(
                "An INSERT statement returning the generated value for column " +
                    "'${column.table.tableName}.${column.name}' is not supported",
                dialect
            )
        }
        return row[column]
     ...
     * the statement was modified. Reading this property materializes the arguments from current column-value mappings
     * and client side defaults.
     *
     */
