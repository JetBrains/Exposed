...
abstract class SchemaUtilityApi {
    ...
    /** Returns the SQL statements that create this [Index]. */
    protected fun Index.createDdl(): List<String> = createStatement()
    ...
    protected fun tableDdlWithoutExistingSequence(
        ...
        val ddlWithoutExistingSequence = table.ddl
            .filter { statement ->
                if (existingSequence != null) {
                    !statement.lowercase().startsWith("create sequence") || !statement.contains(existingSequence.name)
                } else {
                    true
                }
            }
            .partition { it.startsWith("CREATE ") }
        val indicesDDL = table.indices.flatMap { it.createDdl() }
    ...
    protected fun <C : MutableCollection<String>> Table.mapMissingColumnStatementsTo(
        ...
        if (alterTableAddColumnSupported) {
            ...
            }.forEach { destination.addAll(it.createDdl()) }
