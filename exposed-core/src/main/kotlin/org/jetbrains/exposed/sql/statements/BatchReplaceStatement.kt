...
open class BatchReplaceStatement(
    table: Table,
    private val shouldReturnGeneratedValues: Boolean = true
): ReplaceStatement<List<ResultRow>>(table) {
    ...
    override operator fun <S> set(column: Column<S>, value: S) {
        if (data.size > 1 && column !in data[data.size - 2] && !column.isDefaultable()) {
            throw BatchDataInconsistentException(
                "Can't set $value for ${TransactionManager.current().fullIdentity(column)} because previous insertion can't be defaulted for that column."
            )
        }
