...
class ResultRow(
    ...
    private val database: DatabaseApi? = CoreManager.currentTransactionOrNull()?.db
    ...
    private fun <T> getInternal(expression: Expression<T>, checkNullability: Boolean): T = lookUpCache.cached(expression) {
        ...
        if (checkNullability) {
            if (rawValue == null && expression is Column<*> && expression.dbDefaultValue != null && !expression.columnType.nullable) {
                exposedLogger.warn(
                    "Column ${CoreManager.currentTransaction().fullIdentity(expression)} is marked as not null, " +
