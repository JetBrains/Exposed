...
open class MariaDBDialect : MysqlDialect() {
    ...
    override val supportsCreateSequence: Boolean by lazy {
        CoreManager.currentTransaction().db.isVersionCovers(SEQUENCE_MIN_MAJOR_VERSION, SEQUENCE_MIN_MINOR_VERSION)
    }
    ...
    override val sequenceMaxValue: Long by lazy {
        if (CoreManager.currentTransaction().db.isVersionCovers(11, 5)) {
    ...
    @Suppress("MagicNumber")
    override fun isFractionDateTimeSupported(): Boolean = CoreManager.currentTransaction().db.isVersionCovers(5, 3)
