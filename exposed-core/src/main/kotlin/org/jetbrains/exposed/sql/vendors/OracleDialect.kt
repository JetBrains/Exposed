...
internal object OracleFunctionProvider : FunctionProvider() {
    ...
    override fun <T> jsonExtract(
        ...
        if (path.size > 1) {
            CoreManager.currentTransaction().throwUnsupportedException("Oracle does not support multiple JSON path arguments")
        }
        queryBuilder {
    ...
    override fun update(
        ...
        +" FROM "
        targets.describe(CoreManager.currentTransaction(), this)
        where?.let {
            +" WHERE "
            +it
        }
