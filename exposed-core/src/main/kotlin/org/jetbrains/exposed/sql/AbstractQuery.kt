...
abstract class AbstractQuery<T : AbstractQuery<T>>(
    ...
    /** The stored value for a `OFFSET` clause in this query. */
    var offset: Long = 0
        private set

    /** The number of results that should be fetched when generated by an SQL query in this query. */
    ...
    /** Returns the string representation of a query, generated by appending SQL expressions to a [QueryBuilder]. **/
    ...
    /** Modifies an SQL query to retrieve only distinct results if [value] is set to `true`. */
    abstract fun withDistinct(value: Boolean = true): T

    /** Modifies an SQL query to return only [n] results, starting after the specified [offset]. **/
    override fun limit(n: Int, offset: Long): T = apply {
        limit = n
        this.offset = offset
    } as T

    /** Modifies an SQL query to sort results by the specified [column], according to the provided [order]. **/
    fun orderBy(column: Expression<*>, order: SortOrder = SortOrder.ASC): T = orderBy(column to order)

    /** Modifies an SQL query to sort results according to the provided [order] of expressions. **/
    override fun orderBy(vararg order: Pair<Expression<*>, SortOrder>): T = apply {
        (orderByExpressions as MutableList).addAll(order)
    } as T

    /** Modifies the number of results that should be fetched when generated by an SQL query. */
