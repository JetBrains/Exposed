...
suspend fun <T : Table> T.insert(
    ...
    return org.jetbrains.exposed.v1.r2dbc.sql.statements.InsertSuspendExecutable(stmt).apply { execute(TransactionManager.current()) }.statement
}
...
suspend fun <Key : Any, T : IdTable<Key>> T.insertAndGetId(
    ...
    val stmt = buildStatement { insert(body) }
    return org.jetbrains.exposed.v1.r2dbc.sql.statements.InsertSuspendExecutable(stmt).run {
...
suspend fun <T : Table> T.insertIgnore(
    ...
    return org.jetbrains.exposed.v1.r2dbc.sql.statements.InsertSuspendExecutable<Long, InsertStatement<Long>>(stmt).apply {
        execute(TransactionManager.current())
    }.statement
}
...
suspend fun <Key : Any, T : IdTable<Key>> T.insertIgnoreAndGetId(
    ...
    val stmt = buildStatement { insertIgnore(body) }
    return org.jetbrains.exposed.v1.r2dbc.sql.statements.InsertSuspendExecutable<Long, InsertStatement<Long>>(stmt).run {
